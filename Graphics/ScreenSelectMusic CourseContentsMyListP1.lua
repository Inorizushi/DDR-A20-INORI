	...
	Display = Def.ActorFrame { 
		...
		Def.Quad { 
			SetSongCommand=function(self, params)
				...
				self:diffusealpha(1);
				--(cmd(finishtweening;diffusealpha,0;sleep,0.125*params.Number;linear,0.125;diffusealpha,1;linear,0.05;glow,color("0,0,0,0.5");decelerate,0.1;glow,color("0,0,0,0")))(self);
			end;
		};
		...
		--	InitCommand=cmd(x,-133+40;y,0;shadowlength,0;diffuse,color( "0,0,0,1" );strokecolor,color("#999999"));
		--	SetSongCommand=function(self, params) 
		...
		Def.Quad {
 		...
 		LoadFont("_geo 95 20px") .. {
			Text="0";
			InitCommand=cmd(zoomx,1.4;zoomy,1.2;x,130+45;y,-12+88;diffuse,color( "1,1,1,1" );strokecolor, color("0,0,0,0.8"));
			--OnCommand=cmd();
			SetSongCommand=function(self, params)
				if params.PlayerNumber ~= GAMESTATE:GetMasterPlayerNumber() then return end
				self:settext(0);
				self:settext( params.Meter );
				if params.Meter == "?" then
					self:settext("?");
				end;
