...
local function setGradeP1(self,param)
  ...
  if self.ParamCourse then
    local trails = GAMESTATE:GetCurrentTrail(PLAYER_1)
    if trails then
		local sDiff = trails:GetDifficulty()
		
		local diffname = GAMESTATE:GetCurrentTrail(PLAYER_1):GetDifficulty()
...
t[#t+1] = Def.ActorFrame{
	...
	Def.ActorFrame{
		...
		LoadActor("AllCleared_All")..{
			...
			CurrentTrailP1ChangedMessageCommand=function(self) setGradeP1(self) end;
