...
function ReadTotalEXScoreP2( PlayerUID )
...
function SaveTotalEXScoreP2( PlayerUID , extotal)
	local EXScoreFile1 = RageFileUtil:CreateRageFile();
...
ReadTotalEXScoreP2(PlayerUID);


exbefore = math.round(exbefore);
local add_exscore = (STATSMAN:GetCurStageStats():GetPlayerStageStats(PLAYER_2):GetPossibleDancePoints())*(STATSMAN:GetCurStageStats():GetPlayerStageStats(PLAYER_2):GetPercentDancePoints())
add_exscore = math.round(add_exscore);

exsum = exbefore + add_exscore;

SaveTotalEXScoreP2(PlayerUID , exsum);
...
t[#t+1] = Def.RollingNumbers {
	File = THEME:GetPathF("ScreenEvaluation", "EXSummary");
			--InitCommand=cmd(x,SCREEN_CENTER_X-310+10;y,SCREEN_CENTER_Y-52-11+3.2;zoomx,0.947;zoomy,0.958;player,PLAYER_2;playcommand,"Set");
	InitCommand=function(self)
		self:player(PLAYER_2);
		self:x(SCREEN_CENTER_X+636);
