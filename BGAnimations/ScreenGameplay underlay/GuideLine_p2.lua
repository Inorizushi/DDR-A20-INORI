...
for i = 0,800,1 do
	...
	t[#t+1] = Def.Quad{
		...
		InitCommand = function(self)
			...
			self:y(SCREEN_TOP+offsetY+i*UnitBySpeedModP2(GetCurrentSpeedModP2()));
		end;

		OnCommand = function(self)
			local curY=SCREEN_TOP+offsetY+i*UnitBySpeedModP2(GetCurrentSpeedModP2());
			self:y(curY);
		end;
		...
		CodeMessageCommand=function(self, params)
			if params.PlayerNumber == PLAYER_2 then
				...
				if params.Name == "SpeedUp" then
					local curY=SCREEN_TOP+offsetY+i*UnitBySpeedModP2(GetUpperSpeedP2(GetCurrentSpeedModP2()));
					self:y(curY);
				elseif params.Name == "SpeedDown" then
					local curY=SCREEN_TOP+offsetY+i*UnitBySpeedModP2(GetLowerSpeedP2(GetCurrentSpeedModP2()));
					self:y(curY);
				end;
			end;
		end;
...
--				self:y(SCREEN_TOP+100+i*(4*UnitBySpeedModP2()));
...
--				self:y(SCREEN_TOP+offsetY+i*4*UnitBySpeedModP2()+50);
--				self:sleep(0.05);
--				self:queuecommand("Update");
--			end;
--
...
local function GLScroll(self)
	...
	if song then
		...
		self:y(cur*(0-UnitBySpeedModP2(GetCurrentSpeedModP2())));
...
function GetUpperSpeedP2(cur_hispeed)
...
function GetLowerSpeedP2(cur_hispeed)
